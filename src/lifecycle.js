...
export class CareLifecycle {
  ...
  async buildMemory(observations,options={}){
    ...
    return {phase:'memory_build',subject_id:runs[0].subject_id,cleared,processed:runs.length,source_observations:observations.length,runs,memory:this.store.statesFor(runs[0].subject_id)};
  }
  ...
  async converse(rawObservation,options={}){
    ...
    return {phase:'conversation',conversation,feedback,writes,patient_memory_written:writes[0].committed,doctor_memory_written:writes[1].committed,memory_ready_for_next_turn:writes.every(x=>x.committed),memory:this.store.statesFor(observation.subject_id)};
  }
}
