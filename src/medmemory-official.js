...
function matchEntityAnswer(output,answer,normalizedOutput=normalizeOfficialText(output)){
  ...
  if(expectedParts.latin.length&&isMultisetSubset(expectedParts.latin,actualParts.latin)&&expectedParts.non_latin&&actualParts.non_latin.includes(expectedParts.non_latin))return{answer,matched:true,kind:'acronym_name_order_equivalence'};
...
function isMultisetSubset(expected,actual){const remaining=[...actual];for(const value of expected){const index=remaining.indexOf(value);if(index<0)return false;remaining.splice(index,1);}return true;}
function normalizeOfficialText(value){const punctuation=new Set(Array.from(`!"#$%&'()*,-./:;<=>?@[\\]^_\`{|}~，。！？、；：""（）【】《》·…—～－–·`)),text=String(value||'').normalize('NFKC').replace(/\s/gu,'').replace(/(?<=\d)(?:到|至)(?=\d)/gu,'');return[...text].filter(character=>!punctuation.has(character)).join('').trim().toLowerCase();}
