...
const route=async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host}`),path=url.pathname;
  try{
    ...
    if(req.method==='GET'&&['/api/states','/api/patient-graphs'].includes(path))return json(res,200,{representation:'patient_graph',subjects:store.memorySubjects()});
    if(req.method==='GET'&&/^\/api\/(?:states|patient-graphs)\/[^/]+$/.test(path)){const subject_id=decodeURIComponent(path.split('/').at(-1)),patient_graph=store.patientGraphFor(subject_id);return json(res,200,{subject_id,patient_graph,states:patient_graph.nodes,graph_edges:patient_graph.edges,state_scope:store.memoryStateScope(subject_id)});}
    ...
    if(req.method==='DELETE'&&/^\/api\/memory\/[^/]+$/.test(path)){const subject_id=decodeURIComponent(path.split('/').at(-1)),before=store.patientGraphFor(subject_id),removed_nodes=store.clearMemory(subject_id);return json(res,200,{ok:true,subject_id,removed_nodes,removed_edges:before.edges.length,removed_states:removed_nodes});}
