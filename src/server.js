...
import { Store } from './db.js'; import { Pipeline } from './pipeline.js'; import { ExperimentHarness } from './experiments.js'; import { ModelGateway } from './gateway.js'; import { ModelRegistry } from './model-registry.js'; import { DebugRuns } from './debug-runs.js'; import { CareLifecycle } from './lifecycle.js'; import { buildVersion } from './version.js'; import { sanitizeSecrets } from './schema.js';

const port=Number(process.env.PORT||8766),store=new Store(),models=new ModelRegistry(store),harness=new ExperimentHarness(store,undefined,models),debugRuns=new DebugRuns(store,models),lifecycle=new CareLifecycle(store,models),publicDir=resolve('public');
...
const route=async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host}`),path=url.pathname;
  try{
    ...
    if(req.method==='GET'&&path==='/api/states')return json(res,200,{subjects:store.memorySubjects()});
    if(req.method==='GET'&&/^\/api\/states\/[^/]+$/.test(path)){const subject_id=decodeURIComponent(path.split('/').at(-1));return json(res,200,{subject_id,states:store.statesFor(subject_id),state_scope:store.memoryStateScope(subject_id)});}
    if(req.method==='GET'&&/^\/api\/memory\/scope\/[^/]+$/.test(path)){const subject_id=decodeURIComponent(path.split('/').at(-1));return json(res,200,{subject_id,state_scope:store.memoryStateScope(subject_id)});}
    if(req.method==='DELETE'&&/^\/api\/memory\/[^/]+$/.test(path)){const subject_id=decodeURIComponent(path.split('/').at(-1));return json(res,200,{ok:true,subject_id,removed_states:store.clearMemory(subject_id)});}
    ...
    if(req.method==='GET'&&path==='/api/experiments/summaries')return json(res,200,harness.listSummaries(url.searchParams.get('benchmark')||null));
    if(req.method==='GET'&&path==='/api/experiments')return json(res,200,harness.listSummaries());
