...
export class Store {
  ...
  memoryGraphFor(subjectId) { const nodes=this.memoryNodesFor(subjectId),edges=this.memoryEdgesFor(subjectId),by_family=Object.fromEntries(['BC','PE','PA','CS','CP','LO'].map(family=>[family,nodes.filter(node=>(node.families||[]).includes(family)).length])),byEpisode=new Map();for(const node of nodes){const episode=String(node.episode_id||'').trim();if(!episode)continue;const ids=byEpisode.get(episode)||[];ids.push(String(node.memory_id));byEpisode.set(episode,ids);}const episode_memberships=[...byEpisode].map(([episode_id,memory_ids])=>({episode_id,memory_ids}));return{version:'careharness-memory-graph.v2-semantic-state',subject_id:subjectId,nodes,edges,episode_memberships,summary:{node_count:nodes.length,edge_count:edges.length,episode_membership_count:episode_memberships.length,by_family,temporal_edge_count:edges.filter(edge=>edge.edge_family==='temporal').length,clinical_care_edge_count:edges.filter(edge=>edge.edge_family==='clinical_care').length,context_edge_count:edges.filter(edge=>edge.edge_family==='context'||edge.relation_type==='co_observed').length}}; }
