...
function policySatisfied(context,task){const selected=context.action_policy?.selected_actions||context.action_trace?.map(item=>item.action)||[],required=['focus','verify','answer'];if(task==='state_update')required.push('anchor','discriminate','reconcile');else if(task==='multiple_choice'){required.push('contrast');if(String(context.working_state?.temporal?.operator||context.working_state?.control_decisions?.time?.operator||'none')!=='none')required.push('reconcile');}else if(TEMPORAL_TASKS.has(task)||task==='entity_exact_match')required.push('anchor','discriminate');if(COMPLEX_TASKS.has(task))required.push('anchor','connect','evaluate');return required.every(action=>selected.includes(action))&&selected.at(-1)==='answer'&&selected.length<=Number(context.action_policy?.action_budget||Infinity);}
