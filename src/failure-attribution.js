export const FAILURE_TAXONOMY_VERSION='careharness-unified-memory-failure-taxonomy.v2';

export const FAILURE_TAXONOMY=deepFreeze({
  H1:{axis:'harness',name:'memory_node_extraction_or_indexing',component:'Memory Node'},
  H2:{axis:'harness',name:'memory_graph_versioning',component:'Memory Graph'},
  H3:{axis:'harness',name:'working_memory_search_or_refinement',component:'search/refine'},
  H4:{axis:'harness',name:'temporal_trace_or_version_resolution',component:'trace'},
  H5:{axis:'harness',name:'trace_or_semantic_assessment',component:'trace/assess'},
  H6:{axis:'harness',name:'memory_node_or_edge_verification',component:'verify'},
  H7:{axis:'harness',name:'investigation_policy_budget_or_termination',component:'investigation_policy'},
...
export function attributeFailure({score,raw_observations=[],all_memory_nodes=[],runtime_context=null}){
  ...
  const rawCheck=stageCheck('raw_session',raw_observations,item=>item.raw_text,terms,item=>item.observation_id||`${item.episode_id}/${item.turn_id}`),
    memoryCheck=stageCheck('Memory_Node',all_memory_nodes,item=>item.text,terms,item=>item.memory_id),
    workingMemory=[...profileItems(context.patient_profile),...(context.recent_sessions||[]).map(session=>({memory_id:`recent:${session.episode_id}`,text:session.transcript||''})),...(context.memory_nodes||[])],workingCheck=stageCheck('Answer_Context',workingMemory,item=>item.text,terms,item=>item.memory_id),
  ...
  const checks=[rawCheck,memoryCheck,workingCheck,relationCheck,verifyCheck,answerCheck,scorerCheck];
  ...
  else if(rawCheck.supported&&!memoryCheck.supported){code='H1';reason='support exists in a visible raw session but is absent from Memory Nodes';}
  else if(memoryCheck.supported&&!workingCheck.supported){code='H3';reason='support exists in the Memory Graph but search/refine did not place it in Working Memory';}
  ...
  else if(!verifyCheck.satisfied){code='H6';reason='Working Memory nodes or edges failed verification';}
...
function relationStage(context,task,score){const relations=context.memory_edges||[],trace=context.investigation_trace||[],selected=new Set(trace.map(item=>item.worker)),semanticTrace=semanticRelationEvaluatorTrace(context,score),dated=new Set((context.memory_nodes||[]).map(item=>item.event_time||item.episode_id).filter(Boolean)),traceSatisfied=!TEMPORAL_TASKS.has(task)||selected.has('context')||selected.has('trace')||dated.size>1,supportSatisfied=!COMPLEX_TASKS.has(task)||Boolean(context.semantic_evaluation&&selected.has('assess'));return{stage:'relation/verify-route',relation_count:relations.length,relation_types:[...new Set(relations.map(item=>item.relation_type))],trace_satisfied:traceSatisfied,support_chain_satisfied:supportSatisfied,semantic_evaluator_status:semanticTrace?.status||'not_run',semantic_evaluator_error_kind:semanticTrace?.error_kind||semanticTrace?.model_trace?.error?.kind||null,persistent_graph_used:relations.some(item=>item.persistent===true),persistent_relation_count:relations.filter(item=>item.persistent===true).length,query_local_relation_count:relations.filter(item=>item.persistent===false).length,causal_relation_claimed:relations.some(item=>item.causal_claim===true),post_answer_reference_used:false};}
function verifyStage(context){const verification=context.verification||null,trace=context.investigation_trace||[],verifyAction=trace.find(item=>item.worker==='verify');return{stage:'verify',satisfied:Boolean(verifyAction&&verification?.complete),rejected_memory_count:verification?.rejected_memory_ids?.length??null,post_answer_reference_used:false};}
...
function policySatisfied(context){const selected=(context.investigation_trace||[]).map(item=>item.worker).filter(Boolean);return selected.at(-1)==='answer'&&Boolean(context.verification?.complete)&&Boolean((context.memory_nodes||[]).length);}
function actionExecutionFailure(context){return(context.investigation_trace||[]).some(item=>item.error||item.status==='failed');}
...
function profileItems(profile){return(profile?.sections||[]).flatMap((section,sectionIndex)=>(section?.items||[]).map((item,itemIndex)=>({memory_id:`profile:${section?.key||sectionIndex}:${itemIndex}`,text:String(item?.text||'')}))).filter(item=>item.text);}
