...
import { canonicalCalendarDate as canonicalTraceDate } from './temporal-expressions.js';
...
export function searchMemory(request,memoryNodes,memoryEdges,instruction,options={}){return retrieveMemoryCandidates({question_request:createQuestionRequest(request),instruction},memoryNodes,{memory_edges:memoryEdges,limit:options.limit,semantic_scores:options.semantic_scores,semantic_top_k:options.semantic_top_k,episode_diversity:options.episode_diversity===true,candidate_reranker:options.candidate_reranker,dense_admission_features:options.dense_admission_features,aspect_top_k:options.aspect_top_k,strict_fact_diversity:options.strict_fact_diversity===true});}
...
export function refineWorkingMemory(request,memoryNodes=[],memoryEdges=[],instruction={},options={}){
  if(options.evidence_preserving===true){
    const protectedIds=new Set(strings(instruction.memory_ids)),hardInstruction=conservativeRefineInstruction(instruction),hasHardTemporal=Object.keys(hardInstruction).length>0,temporallyScoped=hasHardTemporal?searchMemory(request,memoryNodes,memoryEdges,hardInstruction,{limit:Math.max(memoryNodes.length,Number(options.limit)||0)}).memory_nodes:[...memoryNodes],source=temporallyScoped.length?temporallyScoped:memoryNodes,selected=[];
    for(const node of source){const duplicateIndex=selected.findIndex(item=>evidenceEquivalent(item,node));if(duplicateIndex<0){selected.push(node);continue;}const incumbent=selected[duplicateIndex],incumbentProtected=protectedIds.has(String(incumbent.memory_id)),candidateProtected=protectedIds.has(String(node.memory_id));if(candidateProtected&&!incumbentProtected)selected[duplicateIndex]=node;}
    const ids=new Set(selected.map(node=>String(node.memory_id))),removed=memoryNodes.filter(node=>!ids.has(String(node.memory_id))).map(node=>node.memory_id),duplicateRemoved=source.filter(node=>!ids.has(String(node.memory_id))).map(node=>node.memory_id),temporalRemoved=memoryNodes.filter(node=>!source.some(item=>item.memory_id===node.memory_id)).map(node=>node.memory_id);
    return{memory_nodes:selected,memory_edges:connectWorkingMemory(selected,memoryEdges),trace:{version:'careharness-evidence-preserving-refine.v1',evidence_preserving_refine:true,refinement_applied:removed.length>0,removed_memory_ids:removed,duplicate_removed_memory_ids:duplicateRemoved,hard_temporal_mismatch_memory_ids:temporalRemoved,protected_memory_ids:[...protectedIds],removal_policy:'near_duplicate_or_explicit_hard_temporal_mismatch_only'}};
  }
...
function evidenceEquivalent(left,right){
  const leftEpisode=String(left?.episode_id||left?.observation_id||''),rightEpisode=String(right?.episode_id||right?.observation_id||'');if(String(left?.subject_id||'')!==String(right?.subject_id||'')||!leftEpisode||leftEpisode!==rightEpisode)return false;
  const normalize=value=>String(value||'').normalize('NFKC').toLowerCase().replace(/^(?:patient|doctor|患者|医生)(?:原话|陈述|报告|建议|解释)?[：:]?/u,'').replace(/[\s\p{P}\p{S}]+/gu,''),a=normalize(left?.source_text||left?.text),b=normalize(right?.source_text||right?.text);if(!a||!b)return false;if(a===b)return true;const shorter=Math.min(a.length,b.length),longer=Math.max(a.length,b.length);return shorter>=12&&(a.includes(b)||b.includes(a))&&shorter/Math.max(1,longer)>=.82;
}
...
export function validateInvestigationAssessment(value,options={}){
  ...
  const structured=options.structured_evidence_ledger===true,missing_information=array(value.missing_information||value.missing_memory).map(String).filter(Boolean).slice(0,structured?8:2),consistentAssessment=assessment==='supported'&&missing_information.length?'partial':assessment;
  const answerFocusLimit=boundedInteger(options.answer_focus_limit,1,16,16);
  return{assessment:consistentAssessment,relevant_memory_ids:strings(value.relevant_memory_ids).slice(0,structured?80:20),covered_aspects:strings(value.covered_aspects).slice(0,10),answer_focus:array(value.answer_focus).slice(0,answerFocusLimit),role_coverage:array(value.role_coverage).slice(0,structured?64:16),connections:array(value.connections||value.relations).slice(0,structured?40:10),reasoning_hypotheses:array(value.reasoning_hypotheses).slice(0,3),missing_information,...(structured?{occurrence_candidates:array(value.occurrence_candidates).slice(0,80),counting:{unit:['admission','event','site'].includes(value.counting?.unit)?value.counting.unit:null,scope_complete:value.counting?.scope_complete===true}}:{})};
}

export function sanitizeInvestigationAssessment(value,options={}){
  const clean=validateInvestigationAssessment(value,options);
  if(options.structured_evidence_ledger===true){
    for(const key of ['answer_focus','role_coverage'])clean[key]=clean[key].map(item=>item&&({...item,source_refs:unique([...strings(item.source_refs),...strings(item.source_ref?[item.source_ref]:[])])}));
  }
  return{...clean,answer_focus:clean.answer_focus.filter(Boolean).map(item=>({aspect:String(item.aspect||''),role:String(item.role||'target'),source_refs:strings(item.source_refs).slice(0,8),memory_ids:strings(item.memory_ids).slice(0,8),required_in_answer:item.required_in_answer!==false})).filter(item=>item.aspect&&(item.source_refs.length||item.memory_ids.length)),role_coverage:clean.role_coverage.filter(Boolean).map(item=>({role:String(item.role||'target').slice(0,80),status:['covered','partial','missing','contradicted'].includes(String(item.status))?String(item.status):'partial',claim:String(item.claim||item.aspect||'').slice(0,300),source_refs:strings(item.source_refs).slice(0,8),memory_ids:strings(item.memory_ids).slice(0,8),missing_detail:String(item.missing_detail||'').slice(0,240)})).filter(item=>item.role&&(item.status==='missing'?item.missing_detail||item.claim:item.claim)),connections:clean.connections.filter(Boolean).map(item=>({from_memory_id:String(item.from_memory_id||''),to_memory_id:String(item.to_memory_id||''),relation_type:String(item.relation_type||'associated_with'),assessment:['supports','contradicts','unresolved'].includes(item.assessment)?item.assessment:'unresolved',supporting_memory_ids:strings(item.supporting_memory_ids).slice(0,8),confidence:probability(item.confidence)})).filter(item=>item.from_memory_id&&item.to_memory_id&&item.from_memory_id!==item.to_memory_id),reasoning_hypotheses:clean.reasoning_hypotheses.filter(Boolean).map((item,index)=>({hypothesis_id:String(item.hypothesis_id||`hypothesis_${index+1}`),grounding_scope:normalizeHypothesisGroundingScope(item.grounding_scope),summary:String(item.summary||''),supporting_source_refs:strings(item.supporting_source_refs).slice(0,8),counter_source_refs:strings(item.counter_source_refs).slice(0,8),supporting_memory_ids:strings(item.supporting_memory_ids).slice(0,8),counter_memory_ids:strings(item.counter_memory_ids).slice(0,8),reasoning_steps:strings(item.reasoning_steps).slice(0,6),confidence:probability(item.confidence)}))};
}

export function applyInvestigationAssessment(questionRequest,baseline,evaluation,options={}){
  const clean=sanitizeInvestigationAssessment(evaluation,options),nodeById=new Map(array(baseline.memory_nodes).map(node=>[String(node.memory_id),node])),validIds=new Set(nodeById.keys()),sourceText=sourceReferenceText(baseline,{include_source_text:options.structured_evidence_ledger===true}),validRefs=new Set(sourceText.keys());clean.relevant_memory_ids=clean.relevant_memory_ids.filter(id=>validIds.has(id));
  if(options.structured_evidence_ledger===true){
    // These are source-linked event annotations, not persistent patient facts.
    // Metadata comes from the cited nodes; Policy cannot invent an admission.
    clean.occurrence_candidates=groundOccurrenceCandidates(clean.occurrence_candidates,nodeById);
    clean.relevant_memory_ids=unique([...clean.relevant_memory_ids,...clean.occurrence_candidates.flatMap(item=>item.memory_ids)]);
  }
  ...
  clean.role_coverage=clean.role_coverage.map(item=>{const citedIds=item.source_refs.filter(ref=>ref.startsWith('memory:')).map(ref=>ref.slice(7)).filter(id=>validIds.has(id)),memory_ids=unique([...item.memory_ids.filter(id=>validIds.has(id)),...citedIds]),source_refs=unique([...item.source_refs.filter(ref=>validRefs.has(ref)),...memory_ids.map(id=>`memory:${id}`)]),sources=source_refs.map(ref=>sourceText.get(ref)).filter(Boolean);if(item.status==='missing')return{...item,memory_ids:[],source_refs:[]};if(!source_refs.length||!claimMatchesSource(item.claim,sources)){rejectedGroundedClaims++;return null;}return{...item,memory_ids,source_refs};}).filter(Boolean);
  if(!clean.role_coverage.length)clean.role_coverage=clean.answer_focus.map(item=>({role:item.role||'target',status:'covered',claim:item.aspect,source_refs:item.source_refs,memory_ids:item.memory_ids,missing_detail:''}));
  for(const gap of clean.missing_information)if(!clean.role_coverage.some(item=>item.status==='missing'&&item.missing_detail===gap))clean.role_coverage.push({role:'unresolved',status:'missing',claim:'',source_refs:[],memory_ids:[],missing_detail:gap});
  const roleGaps=clean.role_coverage.filter(item=>item.status==='missing'||item.status==='partial').map(item=>item.missing_detail||`Incomplete evidence role: ${item.role}`);if(roleGaps.length){clean.missing_information=unique([...clean.missing_information,...roleGaps]).slice(0,options.structured_evidence_ledger===true?8:2);if(clean.assessment==='supported')clean.assessment='partial';}
  enforceCrossAdmissionCoverage(questionRequest,clean,nodeById,options);
  ...
  if(rejectedGroundedClaims){if(clean.assessment==='supported')clean.assessment='partial';clean.missing_information=unique([...clean.missing_information,INVESTIGATION_UNGROUNDED_CLAIM_GAP]).slice(0,2);if(options.structured_evidence_ledger===true&&!clean.role_coverage.some(item=>item.status==='missing'&&item.missing_detail===INVESTIGATION_UNGROUNDED_CLAIM_GAP))clean.role_coverage.push({role:'unresolved_grounding',status:'missing',claim:'',source_refs:[],memory_ids:[],missing_detail:INVESTIGATION_UNGROUNDED_CLAIM_GAP});}
  ...
  if(options.structured_evidence_ledger===true){
    const invalidEvents=clean.occurrence_candidates.some(item=>!item.source_refs.length||item.grounding_complete===false);
    if(invalidEvents&&clean.assessment==='supported')clean.assessment='partial';
    clean.counting.scope_complete=clean.counting.scope_complete&&clean.assessment==='supported'&&!clean.missing_information.length&&!invalidEvents;
  }
  const memoryNodes=array(baseline.memory_nodes),memoryEdges=dedupeEdges(array(baseline.memory_edges)),working=buildWorkingMemory(questionRequest,memoryNodes,memoryEdges,clean),coverageState=dynamicCoverageState(baseline.coverage_state,clean,nodeById);
  return{...baseline,memory_nodes:memoryNodes,memory_edges:memoryEdges,relations:memoryEdges,working_memory:working,coverage_state:coverageState,verification:null,semantic_evaluation:clean};
}

function enforceCrossAdmissionCoverage(questionRequest,assessment,nodeById,options={}){
  const request=createQuestionRequest(questionRequest),cross=request.strategy_namespace==='medlocomo'&&request.scope==='cross_admission',requiresSides=cross&&request.query_type!=='adversarial';if(!requiresSides)return;
  const cited=unique([...assessment.relevant_memory_ids,...assessment.answer_focus.flatMap(item=>item.memory_ids),...assessment.role_coverage.flatMap(item=>item.memory_ids)]).filter(id=>nodeById.has(id)),episodes=unique(cited.map(id=>String(nodeById.get(id)?.episode_id||'')).filter(Boolean)),factors=new Map();
  for(const id of cited){const node=nodeById.get(id),factor=String(node?.factor_key||'').trim();if(!factor)continue;const values=factors.get(factor)||new Set();values.add(String(node.episode_id||''));factors.set(factor,values);}
  const alignedFactors=[...factors.entries()].filter(([,values])=>values.size>=2).map(([factor])=>factor),complete=episodes.length>=2&&(alignedFactors.length>0||assessment.connections.some(link=>{const a=nodeById.get(link.from_memory_id),b=nodeById.get(link.to_memory_id);return a&&b&&String(a.episode_id)!==String(b.episode_id);}));
  assessment.cross_admission_coverage={required:true,complete,covered_episode_ids:episodes,aligned_factor_keys:alignedFactors};if(complete)return;
  const gap='Retrieve the same answer-relevant factor from at least one additional Admission and preserve both endpoints.';assessment.missing_information=unique([...assessment.missing_information,gap]).slice(0,options.structured_evidence_ledger===true?8:2);if(!assessment.role_coverage.some(item=>item.role==='cross_admission_counterpart'))assessment.role_coverage.push({role:'cross_admission_counterpart',status:'missing',claim:'',source_refs:[],memory_ids:[],missing_detail:gap});assessment.assessment=assessment.assessment==='unresolved'?'unresolved':'partial';
}
function dynamicCoverageState(previous,assessment,nodeById){
  const roles=array(assessment.role_coverage),covered=unique([...assessment.covered_aspects,...roles.filter(item=>item.status==='covered').map(item=>item.claim||item.role)]).slice(0,24),missing=unique([...assessment.missing_information,...roles.filter(item=>['missing','partial','contradicted'].includes(item.status)).map(item=>item.missing_detail||item.role)]).slice(0,12),ids=unique([...assessment.relevant_memory_ids,...assessment.answer_focus.flatMap(item=>item.memory_ids),...roles.flatMap(item=>item.memory_ids)]).filter(id=>nodeById.has(id)),episodes=unique(ids.map(id=>String(nodeById.get(id)?.episode_id||'')).filter(Boolean));
  return{version:'careharness-dynamic-coverage.v1',revision:Math.max(0,Number(previous?.revision)||0)+1,covered_aspects:covered,missing_aspects:missing,supporting_memory_ids:ids,covered_episode_ids:episodes,complete:assessment.assessment==='supported'&&!missing.length,...(assessment.cross_admission_coverage?{cross_admission:assessment.cross_admission_coverage}:{})};
}

function groundOccurrenceCandidates(values,nodeById){
  return array(values).filter(item=>item&&typeof item==='object').map(item=>{
    const requested=unique([...strings(item.source_refs),...(item.source_ref?[String(item.source_ref)]:[]),...strings(item.memory_ids).map(id=>`memory:${id}`)]),refs=requested.filter(ref=>ref.startsWith('memory:')&&nodeById.has(ref.slice(7))),ids=refs.map(ref=>ref.slice(7)),sources=ids.map(id=>nodeById.get(id)),admissions=unique(sources.map(node=>String(node.episode_id||'')).filter(Boolean)),requestedAdmission=String(item.admission_id||''),admission=requestedAdmission|| (admissions.length===1?admissions[0]:''),grounded=refs.length>0&&refs.length===requested.length&&admissions.includes(admission),times=sources.filter(node=>String(node.episode_id)===admission).map(node=>String(node.event_time||'')).filter(Boolean).sort(),status=['documented','planned','negated','uncertain'].includes(item.event_status)?item.event_status:'uncertain';
    return{event_key:String(item.event_key||'').slice(0,160),admission_id:grounded?admission:'',event_time:times[0]||null,site:String(item.site||'').slice(0,120),event_status:status,source_refs:refs,memory_ids:ids,included:grounded&&item.included===true&&status==='documented',exclusion_reason:grounded?String(item.exclusion_reason||'').slice(0,240):'unresolved_source_reference_or_admission',grounding_complete:grounded,interpretation_only:true};
  });
}

function sourceReferenceText(baseline,{include_source_text=false}={}){
  const out=new Map();
  ...
  for(const node of array(baseline.memory_nodes)){const text=include_source_text?String(node?.source_text||node?.text||''):visibleSourceText(node,{derive_literal_supplement:true});if(node?.memory_id&&text)out.set(`memory:${node.memory_id}`,text);}
  return out;
}
