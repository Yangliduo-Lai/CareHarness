import { retrieveStateCandidates } from './retrieval.js';
import { CAREHARNESS_ACTIONS,CAREHARNESS_RUNTIME_CONTROLS } from './careharness-contract.js';

export const CAREHARNESS_ACTION_POLICY_VERSION='careharness-configured-action-policy.v4';
...
export function executeCareHarnessPolicy(queryPlan,states=[],evidence=[],options={}){
  ...
  let context={states:[],evidence:[],candidates:[],evidence_chains:[],trace:{}},relations=[],proof=null,verification=null;
  const actionTrace=[];
  for(const action of selectedActions){
    const before={states:context.states.length,evidence:context.evidence.length,relations:relations.length};
    if(action==='focus')context=retrieveStateCandidates(queryPlan,states,evidence,{limit:candidateBudget,gate_limit:candidateBudget,graph_edges:graphEdges});
      ...
      context={...context,states:verification.states,evidence:collectEvidence(verification.states,evidenceById)};
  ...
  return{
    states:context.states,evidence:context.evidence,candidates:context.candidates||[],evidence_chains:context.evidence_chains||[],
...
export function verifyWorkingState(states=[],relations=[],evidenceById=new Map(),options={}){
  ...
  for(const state of states){
    ...
    if(String(state.status)==='rejected'||(!historicalView&&!currentView&&['resolved','inactive'].includes(String(state.status)))){rejected.push({state_id:state.state_id,reason:'state_not_valid_for_requested_temporal_view',declared_evidence_ids:declared,resolved_evidence_ids:resolved,blocking:true});continue;}
    accepted.push(state);
  }
...
function traceVersionsAndTime(queryPlan,context,allStates,evidenceById,limit,graphEdges=[]){
  const selected=new Map(context.states.map(item=>[String(item.state_id),item])),byId=new Map(allStates.map(item=>[String(item.state_id),item])),selectedEvidence=new Set(context.states.flatMap(item=>(item.evidence_ids||[]).map(String)));
  ...
  const states=[...selected.values()];
  return{...context,states,evidence:collectEvidence(states,evidenceById),trace:{...context.trace,trace_action:{expanded_state_count:states.length-context.states.length,temporal_operator:queryPlan.temporal_operator||'none',persistent_graph_used:traversableEdges.length>0,traversed_edge_count:traversableEdges.filter(edge=>selectedGraphIds.has(String(edge.from_state_id))||selectedGraphIds.has(String(edge.to_state_id))).length,rejected_edge_count:graphEdges.length-traversableEdges.length}}};
}
...
function desiredCareHarnessActions(queryPlan={}){const task=String(queryPlan.query_type||queryPlan.task||'generic'),temporal=TEMPORAL_TASKS.has(task)||String(queryPlan.temporal_operator||'none')!=='none';return COMPLEX_TASKS.has(task)?['focus','trace','connect','evaluate','verify','answer']:temporal?['focus','trace','verify','answer']:['focus','verify','answer'];}
...
function actionControl(action){if(action==='focus')return'scope';if(action==='trace')return'time';if(action==='connect')return'relation';if(action==='evaluate')return'hypothesis';if(action==='verify')return'grounding';return'termination';}
