...
const rows=loadCompletedRows([args.output,...args.resume_from],caseIds);
...
function loadCompletedRows(paths,caseIds){const allowed=new Set(caseIds),byCase=new Map();for(const path of paths){const jsonPath=resolve(path);if(!existsSync(jsonPath))continue;try{const payload=JSON.parse(readFileSync(jsonPath,'utf8'));for(const row of Array.isArray(payload.cases)?payload.cases:[])if(row?.status==='completed'&&allowed.has(row.case_id)&&!byCase.has(row.case_id))byCase.set(row.case_id,row);}catch(error){process.stderr.write(`CPCD checkpoint ${jsonPath} ignored because it could not be parsed: ${error.message}\n`);}}return[...byCase.values()].sort((a,b)=>caseIds.indexOf(a.case_id)-caseIds.indexOf(b.case_id));}
...
function parseArgs(argv){const values={database:process.env.CAREHARNESS_DB_PATH||'./data/careharness.sqlite',output:'reports/cpcd-qwen37-full.json',resume_from:[],api_key_env:'CAREHARNESS_MATCHED_API_KEY',case_concurrency:4,query_concurrency:4,preprocess_concurrency:4,retries:2,seed:42,cases:[]};for(let index=0;index<argv.length;index++){const key=argv[index],value=argv[index+1];if(key==='--database'){values.database=required(value,key);index++;}else if(key==='--output'){values.output=required(value,key);index++;}else if(key==='--resume-from'){values.resume_from.push(required(value,key));index++;}else if(key==='--api-key-env'){values.api_key_env=required(value,key);index++;}else if(key==='--case-concurrency'){values.case_concurrency=bounded(value,key,1,8);index++;}else if(key==='--query-concurrency'){values.query_concurrency=bounded(value,key,1,16);index++;}else if(key==='--preprocess-concurrency'){values.preprocess_concurrency=bounded(value,key,1,16);index++;}else if(key==='--retries'){values.retries=bounded(value,key,0,3);index++;}else if(key==='--seed'){values.seed=integer(value,key);index++;}else if(key==='--cases'){values.cases=required(value,key).split(',').map(item=>item.trim()).filter(Boolean);index++;}else throw new Error(`Unknown argument: ${key}`);}return values;}
