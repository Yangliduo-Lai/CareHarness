...
const caseIds=args.cases.length?args.cases:catalog.cases,rows=[];
try{
  await pool(caseIds,args.case_concurrency,async caseId=>{
    ...
    if(!experiment||experiment.status!=='completed')throw lastError||new Error(`CPCD case ${caseId} did not complete`);
    ...
    rows.push(row);process.stderr.write(`CPCD case ${caseId} completed: ${row.scored}/${row.query_count}, score=${format(row.average_score)}\n`);
  });
  rows.sort((a,b)=>caseIds.indexOf(a.case_id)-caseIds.indexOf(b.case_id));
  const payload={report_version:'cpcd-careharness-results.v1',generated_at:new Date().toISOString(),benchmark:'cpcdbench',model:{provider:model.provider,base_url:model.base_url,model:model.model,temperature:model.temperature,max_tokens:model.max_tokens},case_count:rows.length,query_count:rows.reduce((sum,row)=>sum+row.query_count,0),scored:rows.reduce((sum,row)=>sum+row.scored,0),failed:rows.reduce((sum,row)=>sum+row.failed,0),average_score:weightedAverage(rows,'average_score','scored'),by_task:mergeTaskSummaries(rows),cases:rows};
