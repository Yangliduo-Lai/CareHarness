...
test('matched manifest freezes the unified graph and adaptive investigation architecture',()=>{
  ...
  assert.equal(left.models.investigation_policy.model,'same-model');assert.equal(left.models.relation_evaluator.model,left.models.answer.model);assert.equal(left.models.embedding.base_model,'BAAI/bge-small-zh-v1.5');
  assert.equal(left.prompt_versions.memory_extractor,'extractor.session-memory.v13-source-anchored-semantic-state');assert.equal(left.prompt_versions.memory_relation_classifier,'memory-relation-classifier.v1-source-grounded-noncausal');assert.equal(left.prompt_versions.investigation_policy,'careharness-investigation-policy.closed-loop.v30-quality-weighted-learning');assert.equal(left.prompt_versions.investigation_strategy,'medmemory-investigation-strategy.v2-auditable-offline-student');
  assert.equal(left.method_claims.persistent_unified_memory_graph,true);assert.equal(left.method_claims.policy_owned_investigation_state,true);assert.equal(left.method_claims.static_query_preanalysis,false);assert.equal(left.method_claims.transparent_query_type_adaptation,true);assert.equal(left.method_claims.learned_policy_claimed,false);assert.equal(left.method_claims.offline_student_policy_loaded,true);assert.deepEqual(left.action_policy_learning,{enabled:false});assert.deepEqual(left.action_policy_exploration,{enabled:false,rate:0,seed:0,training_only:true});
