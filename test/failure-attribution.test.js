...
test('counterfactual audit assigns H1 when raw Session contains target but Memory Nodes do not',()=>{
  const result=attributeFailure({score:baseScore,raw_observations:[{observation_id:'o1',raw_text:'患者正在服用二甲双胍。'}],all_memory_nodes:[{memory_id:'m1',text:'患者血糖偏高。'}],runtime_context:{memory_nodes:[],investigation_trace:[]}});
  assert.equal(result.code,'H1');
  assert.deepEqual(result.counterfactual_checks.map(item=>item.stage),['raw_session','Memory_Node','Answer_Context','relation/verify-route','verify','answer','scorer']);
...
test('a relation-evaluator schema/model failure is attributed to M1 instead of H5',()=>{
  const score={...baseScore,task:'multi_hop_clinical_deduction',retrieval_trace:{semantic_relation_evaluator:{status:'failed',model_calls:1,error:'invalid evaluator payload',error_kind:'schema_error',token_input:30,token_output:4,latency_ms:8}}},runtime={investigation_trace:['search','trace','assess','verify','answer'].map(worker=>({worker,status:'completed'})),memory_nodes:[{memory_id:'m1',text:'患者记录。'}],memory_edges:[],semantic_evaluation:null,verification:{complete:true}};
