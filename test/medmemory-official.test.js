...
test('appendix Judge templates stay byte-identical to the centralized prompt contract',()=>{
...
test('the IG Answer Prompt is rendered from the appendix without hidden Judge data',()=>{const item=A.medmemorybench.cases(A.medmemorybench.load({persona_id:1,max_session:10,query_type:'inference_generation'}))[0],input={task:item.task,question:'医生，我要不要把降糖药加量？',retrieved_states:[{value:'患者已规律用药。'}],retrieved_evidence:[{text:'患者已规律用药。'}]},prompt=promptFor('medmemory_answer',input),messages=medMemoryAnswerMessages(input);assert.equal(messages[0].content,MEDMEMORY_SHARED_SYSTEM_PROMPT);assert.match(prompt,/considering the patient’s allergy history, medical history, medications, and personal preferences/);assert.match(prompt,/do not give generic medical advice/);assert.match(prompt,/患者已规律用药/);assert.ok(prompt.endsWith('Answer:'));assert.doesNotMatch(prompt,/required_patient_info|common_wrong_answer|expected_answer|Gold|Transport Requirement/);});

test('the MCD Answer Prompt follows the appendix structure',()=>{const item=A.medmemorybench.cases(A.medmemorybench.load({persona_id:1,max_session:100,query_type:'multi_hop_clinical_deduction'}))[0],input={task:item.task,question:item.question,retrieved_states:[{value:'2024-01-15 患者停用恩格列净。'}],retrieved_evidence:[{text:'患者已停用恩格列净。'}]},prompt=promptFor('medmemory_answer',input);assert.match(prompt,/carefully review the patient’s complete medical history/);assert.match(prompt,/combining information from multiple visits/);assert.match(prompt,/Clearly list the memory content/);assert.match(prompt,/clear reasoning path from evidence to conclusions/);assert.match(prompt,/final comprehensive judgment/);assert.match(prompt,/2024-01-15 患者停用恩格列净/);assert.ok(prompt.endsWith('Answer:'));assert.doesNotMatch(prompt,/required_memory_nodes|reasoning_chain|expected_answer|common_wrong_answer/);});
